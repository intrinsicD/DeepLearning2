...
def _safe_l2_normalize(x: torch.Tensor, dim: int = -1, eps: float = 1e-6) -> torch.Tensor:
    """Numerically stable normalization that avoids propagating NaNs."""
...
def _check_finite(named_tensors):
    """Return False if any tensor contains NaN/Inf and report offending name."""
...
def info_nce_loss(query: torch.Tensor, key: torch.Tensor, temperature: float = 0.07) -> torch.Tensor:
    ...
    """
    # Normalize
    query = _safe_l2_normalize(torch.nan_to_num(query))
    key = _safe_l2_normalize(torch.nan_to_num(key))

    # Prevent degenerate temperatures
...
def train_epoch(model, dataloader, optimizer, scaler, device, epoch, args):
    ...
    for batch_idx, batch in enumerate(pbar):
        ...
        with autocast(enabled=args.use_amp):
            ...
            text_emb = torch.nan_to_num(text_outputs['central_latent'])
            image_emb = torch.nan_to_num(image_outputs['central_latent'])
            audio_emb = torch.nan_to_num(audio_outputs['central_latent'])
            ...
            finite, offending = _check_finite(named_embs)
            if not finite:
                pbar.write(
                    f"[WARN] Non-finite embeddings detected in '{offending}' at batch {batch_idx}. Skipping update."
                )
                scaler.update()
                continue
        ...
        if not torch.isfinite(loss):
            pbar.write("[WARN] Non-finite loss detected. Skipping batch.")
            scaler.update()
            continue
...
def evaluate(model, dataloader, device, enable_ttt=False):
    ...
    image_embs = F.normalize(torch.cat(all_image_embs), dim=-1, eps=1e-6)
    text_embs = F.normalize(torch.cat(all_text_embs), dim=-1, eps=1e-6)
    audio_embs = F.normalize(torch.cat(all_audio_embs), dim=-1, eps=1e-6)
...
if __name__ == '__main__':
    ...
    # Output
    parser.add_argument('--output_dir', type=str, default='./results/folder_per_model/multimodal_memory/outputs/flickr8k')
