...
cfg = load_config('modules/nl_mm/configs/tiny_single_gpu.yaml')
model = NLMM(cfg)
batch = {
...
python -c "
import torch
checkpoint = torch.load('results/folder_per_model/multimodal_memory/outputs/best_model.pt', map_location='cpu')
...
# Load checkpoint
checkpoint = torch.load('results/folder_per_model/multimodal_memory/outputs/best_model.pt')
...
```bash
# View JSON metrics
cat results/folder_per_model/nl_mm/outputs/nlmm_flickr8k/metrics.json | python -m json.tool

# Or with jq
cat results/folder_per_model/nl_mm/outputs/nlmm_flickr8k/metrics.json | jq .
```
...
with open('results/folder_per_model/nl_mm/outputs/nlmm_flickr8k/metrics.json') as f:
    metrics = json.load(f)
...
```bash
python modules/nl_mm/export.py \
    --config modules/nl_mm/configs/tiny_single_gpu.yaml \
    --checkpoint results/folder_per_model/multimodal_memory/outputs/best_model.pt \
...
observer = Observer()
observer.schedule(MetricsMonitor(), 'results/folder_per_model/nl_mm/outputs/nlmm_flickr8k', recursive=False)
observer.start()
...
1. Check the training log:
   ```bash
   tail -100 results/folder_per_model/nl_mm/outputs/nlmm_flickr8k/training.log
   ```
