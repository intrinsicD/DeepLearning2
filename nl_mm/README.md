...
The training entrypoint is [`nl_mm/train.py`](train.py).  It constructs an
...
the model-configured optimizer and scheduler stack.【F:nl_mm/train.py†L1-L35】
...
1. **Install dependencies.**  Install PyTorch (CUDA build if you plan to use a
   ...
   allows the sample YAML config to be parsed.【F:nl_mm/utils.py†L1-L22】
2. **Select a configuration.**  The default
   ...
   modality depths, and optimizer hyper-parameters.【F:nl_mm/configs/tiny_single_gpu.yaml†L1-L24】
...
3. **Run training.**  Invoke the module with the chosen config and desired step
   count:
   ```bash
   python -m nl_mm.train --config nl_mm/configs/tiny_single_gpu.yaml --steps 200
   ```
   - The script automatically selects `cuda` when available; otherwise it runs on CPU.【F:nl_mm/train.py†L18-L23】
   - `--steps` overrides `train_steps` inside the configuration, allowing quick
     experiments without modifying the file.【F:nl_mm/train.py†L12-L34】
...
4. **Monitor optimization.**  The `NLMM.configure_scheduler` helper builds the
   ...
   advance every optimizer in lockstep.【F:nl_mm/train.py†L24-L33】
...
Evaluation is orchestrated by [`nl_mm/evaluate.py`](evaluate.py).  It instantiates
the same configuration, switches the model to evaluation mode, and performs a
forward pass with optionally enabled test-time training adapters.【F:nl_mm/evaluate.py†L1-L33】
...
- To disable adapters during evaluation, set `"enable": false` inside the `ttt`
  block of your configuration.【F:nl_mm/evaluate.py†L18-L25】

## Exporting for deployment

[`nl_mm/export.py`](export.py) traces the model with TorchScript for deployment.
It wraps `NLMM` in a lightweight module that exposes a single `forward(text)`
method and saves the traced graph to the path supplied via `--output`.【F:nl_mm/export.py†L1-L35】
