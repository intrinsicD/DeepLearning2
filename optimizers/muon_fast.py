...
# When the Newton--Schulz residual stalls we increase ``eps`` multiplicatively to
# improve the conditioning of the Gram matrix.  The constant 3.0 keeps the total
# increase within a single restart to 4× (baseline ``eps`` plus ``3 * eps``),
# matching the bump factor used in production Muon implementations.
_EPS_BUMP_MULTIPLIER = 3.0
                ...
                if it > 0 and tol > 0.0 and (it % 2 == 1):
                    ...
                    if not eps_bumped and it > 2:
                        if abs(residual - prev_residual) < tol * 0.1:
                            gram = gram + _EPS_BUMP_MULTIPLIER * eps * identity
                            eps_bumped = True
                            restart = True
                            break
